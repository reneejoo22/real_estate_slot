...
② 슬롯 추출 (Prompt Engineering + Information Extraction, LLM Function Calling)
   ...
   ▼
③ pandas 필터링 + 점수 기반 랭킹 (일반 코드, AI 아님)
   ▼
...
### 데이터 생성
- **`generate_synthetic_data.py`** — `slot_schema.json`의 범주형 슬롯(지역/거래유형/주거유형/층조건/근접시설) 조합을 바탕으로 매물 데이터를 합성 생성. 지역별로 다른 가격대를 반영해 그럴듯한 보증금/월세/평수를 랜덤 배정. 각 매물에는 반려동물 가능 여부, 주차, 분위기 등 자유 텍스트 `설명` 컬럼도 포함. 결과물: `synthetic_listings.csv`
- **`synthetic_listings.csv`** — 위 스크립트의 산출물. 챗봇이 실제로 검색 대상으로 사용하는 매물 DB.
- **`generate_test_utterances.py`** — `synthetic_listings.csv`의 매물 정보를 바탕으로, 슬롯 일부만 언급하는 자연어 발화와 그 발화에 실제로 포함된 슬롯 값(정답)을 함께 생성. 슬롯 추출 성능을 평가하기 위한 테스트셋을 만드는 용도. **슬롯 3개 이상인 발화가 전체의 80% 이상이 되도록 강제**해 쉬운 케이스 비중을 줄임. 결과물: `test_cases.csv`
- **`test_cases.csv`** — 위 스크립트의 산출물. `utterance`(입력 문장)와 `true_*`(정답 슬롯 값) 컬럼으로 구성.

### 핵심 파이프라인
- **`langchain_pipeline.py`** — 메인 파이프라인. 의도 분류(`classify_intent`), 슬롯 추출(`extract_slots`), pandas 필터링 + 점수 기반 랭킹(`search_listings`), 결과 포맷팅(`format_results`) 함수를 포함. LLM은 Gemini(`gemini-2.5-flash`)를 사용하며 `.env`의 `GOOGLE_API_KEY`로 인증.
...
### 점수 기반 랭킹 (`search_listings`)
지역/거래유형/주거유형/가격/면적은 **필수 조건**으로 처리해 조건 불일치 매물을 필터링하고,
층조건과 근접시설은 **가산점 조건**으로 처리합니다. 조건을 충족하면 점수를 올려 상위에 노출시키되,
충족하지 못해도 탈락시키지 않아 "완벽한 매물이 없을 때도 가장 가까운 매물"을 보여줄 수 있습니다.
동점이면 월세가 낮은 매물이 우선 노출됩니다.
...
```
============================================================
슬롯별 평가 결과 (15개 기준, gemini-flash-lite-latest)
============================================================
...
- **`기타` 슬롯** (`SlotFilling`에 정의됨): 반려동물, 주차, 채광, 동네 분위기 등 정형 슬롯으로 분류되지 않는 자유 조건을 담기 위한 슬롯. 매물 `설명` 컬럼과의 임베딩 유사도 검색 기반으로 랭킹에 반영하는 구조는 `search_listings`에 이미 구현되어 있으나, 테스트셋(`test_cases.csv`) 생성 및 평가 파이프라인에서는 아직 미사용. 향후 테스트셋에 `기타` 슬롯 케이스를 추가하고 유사도 기반 채점(`SIMILARITY_MATCH_THRESHOLD`)으로 평가하는 것이 목표.